...
import { resolveDistanceM, resolveStartDate } from '../draft'
import { raceDistanceLabel, formatGoalTime } from '../../constants'

const EMPTY = '·'

const formatDate = (d) =>
  d ? new Date(d).toLocaleDateString('fr-FR', { day: 'numeric', month: 'long', year: 'numeric' }) : EMPTY
...
const StepReview = ({ draft, goTo }) => {
  ...
  const distLabel = raceDistanceLabel(distanceM) || (distanceM ? `${(distanceM / 1000).toFixed(1)} km` : EMPTY)
  const goalSec = draft.goalSec != null && draft.goalSec > 0 ? draft.goalSec : null
  ...
  return (
    <Box>
      ...
      <SectionCard title="Course" onEdit={() => goTo(0)}>
        <Row label="Nom" value={draft.name || EMPTY} />
        <Row label="Date" value={formatDate(draft.date)} />
        <Row label="Début d'entraînement" value={formatDate(resolveStartDate(draft))} />
      ...
      <SectionCard title="Forme" onEdit={() => goTo(1)}>
        ...
        <Row label="VMA" value={draft.vmaKmh ? `${draft.vmaKmh} km/h` : EMPTY} />
      ...
      <SectionCard title="Objectif" onEdit={() => goTo(2)}>
        <Row label="Temps cible" value={goalSec != null ? formatGoalTime(goalSec) : EMPTY} />
        {races.length > 0 && (
          <>
            <Divider sx={{ my: 1 }} />
            {races.map((r, i) => (
              <Row key={i} label={r.name} value={r.time || EMPTY} />
            ))}
          </>
        )}
        {draft.notes.trim() && (
